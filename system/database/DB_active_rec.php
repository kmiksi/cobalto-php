...
class CI_DB_active_record extends CI_DB_driver {
     ...
     * @since 16/02/2012
     * Pesquisa os itens, fazendo a paginação quando necessário e enviando o resultado para a grid.<br>
     * Caso haja qualquer erro, a consulta SQL será automaticamente logada nos logs do CodeIgniter.
    ...
    function sendToGrid() {
        ...
        $paramsJqGrid->sortDirection = $_GET['sord'];
        $paramsJqGrid->records = $qtd;

        $paramsJqGrid->limit = isset($_GET['totalrows']) ? $_GET['totalrows'] : $_GET['rows'];
        $paramsJqGrid->total = $qtd > 0 ? ceil($qtd / $paramsJqGrid->limit) : 0;
        $paramsJqGrid->page = $_GET['page'] > $paramsJqGrid->total ? $paramsJqGrid->total : $_GET['page'];
        ...
        if ($qtd > 0) {
            ...
            if ($query === FALSE) {
                log_message('error', "sendToGridERROR on step2 (counted $qtd rows, error on searching itens)");
                logLastSQL();
                $this->_reset_select();
                return FALSE;
            }

            $paramsJqGrid->rows = $query->result();
        }
        $this->_reset_select();
     ...
     */
    function setDate($field, $date = '', $escape = TRUE) {
        if (is_array($field)) {
            foreach ($field as $key => $value) {
                $this->setDate($key, $value, $escape);
            }
            return $this;
        }
     ...
     */
    function likeName($field, $search = '', $side = 'both') {
        $search = array_keys(array_flip(explode(" ", $search)));
